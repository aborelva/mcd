...
SELECT service, COUNT(id_employes) AS 'nombre d employes' FROM employes WHERE service='commercial';

-- 4. Afficher le salaire des commerciaux sur 1 année
SELECT prenom, nom, salaire*12 AS 'salaire annuel' FROM employes WHERE service='commercial';
SELECT service, SUM(salaire*12) AS 'salaire annuel' FROM employes WHERE service='commercial';

-- 5. Afficher le salaire moyen par service
SELECT service, ROUND(AVG(salaire)) AS 'salaire annuel' FROM employes GROUP BY service;

-- 6. Afficher le nombre de recrutement sur 2010
SELECT COUNT(id_employes) AS 'nombre de recrutement sur 2010' FROM employes WHERE date_embauche LIKE '2010%'; 

-- 7. Afficher le nombre de services DIFFERENTS
SELECT COUNT(DISTINCT service) AS 'services existants' FROM employes;

-- 8. Afficher le nombre d'employés par service
SELECT service, COUNT(id_employes) AS 'nombre d employes' FROM employes GROUP BY service; 
